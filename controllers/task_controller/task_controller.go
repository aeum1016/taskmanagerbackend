...
func GetTasks(ctx *gin.Context) ([]models.Task, error) {
	...
	query := `SELECT * FROM tm.tasks WHERE uid=@uid`
	args := pgx.NamedArgs{
		"uid": uid,
	}
...
func AddTask(ctx *gin.Context) (models.Task, error) {
	...
	query := `INSERT INTO tm.tasks (id, uid, title, priority, due_date, description, hours_estimate, tags, completed) VALUES (@id, @uid, @title, @priority, @duedate, @description, @hours, @tags, @completed)`
	args := pgx.NamedArgs{
		"id":          uuid.New(),
		"uid":         uid,
		"title":       newTask.Title,
...
func RemoveCompletedTasks() error {
	...
	_, err := db.Exec(context.Background(), "DELETE FROM tm.tasks WHERE completed = true")
	if err != nil {
		return err
	}
